...
import com.itcalf.renhe.utils.HttpUtil;
   ...
   * @author wangning
   */
public class EditInterestInfoTask  extends BaseAsyncTask<MessageBoardOperation> {
	private Context mContext;
	public EditInterestInfoTask(Context mContext) {
		super(mContext);
	...
	protected MessageBoardOperation doInBackground(String... params) {
		...
		reqParams.put("interests", params[2]);// 
		try
		{
			MessageBoardOperation mb = (MessageBoardOperation) HttpUtil.doHttpRequest(
					Constants.Http.EDIT_INTEREST_INFO, reqParams,
