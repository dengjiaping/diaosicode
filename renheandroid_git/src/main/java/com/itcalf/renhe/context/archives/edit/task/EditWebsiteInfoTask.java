...
import com.itcalf.renhe.utils.HttpUtil;
   ...
   * @author wangning
   */
public class EditWebsiteInfoTask  extends BaseAsyncTask<MessageBoardOperation> {
	private Context mContext;
	private int[] webTypes;
	private String[] webInfos;
	public EditWebsiteInfoTask(Context mContext,int[] webTypes,String[] strings) {
		super(mContext);
		this.mContext = mContext;
		this.webTypes = webTypes;
		this.webInfos = strings;
	}
	@Override
	public void doPre() {
	...
	protected MessageBoardOperation doInBackground(String... params) {
		...
		reqParams.put("webTypes", webTypes);// 
		reqParams.put("webInfos", webInfos);// 
		try
		{
			MessageBoardOperation mb = (MessageBoardOperation) HttpUtil.doHttpRequest(
					Constants.Http.EDIT_WEBSITE_INFO, reqParams,
