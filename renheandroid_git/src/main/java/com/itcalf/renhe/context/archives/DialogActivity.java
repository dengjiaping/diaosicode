...
public class DialogActivity extends BaseActivity {
	...
	private boolean isFromArchieve = false;
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
				...
				if (itemName.contains("renhe")) {
					...
					if (null == filePath) {
						...
						filePath = ExternalStorageUtil.getCacheAvatarPath(DialogActivity.this, getRenheApplication()
...
//					list.add(resolveInfo);
			...
			for (int i = 0; i < mApps.size(); i++) {
				...
				if ((packName.contains("tencent") && name.contains("ShareImgUI")) || packName.contains("email")
						|| packName.contains("mms") || packName.contains("com.tencent.mobileqq")) {
					list.add(resolveInfo);
				}
			}
			list.add(0, innerMsgResolve);
		}
	...
	private void share2Tencent(boolean is2TimeLine) {
		...
		if (is2TimeLine) {
			...
			if (wxSdkVersion < TIMELINE_SUPPORTED_VERSION) {
				Toast.makeText(DialogActivity.this, "抱歉，您的微信版本暂不支持分享到朋友圈", Toast.LENGTH_LONG).show();
				return;
			}
		}
		/**
		...
		if (isFromArchieve) {
			if (mProfile != null && mProfile.getUserInfo() != null) {
				...
				webUrl = "http://r.renhe.cn/"+sharesid;
			} else {
				Toast.makeText(this, "分享失败", Toast.LENGTH_SHORT).show();
				return;
			}
			titleString = this.userName + "的人和网档案";//好友姓名
			contentString = "我分享了" + this.userName
					+ "的人和网档案给你，人和网-人脉成就事业。通过人和网您可以快速找到各种有价值的商务人脉，并且随时随地和您的人脉保持连接！目前人和网上有1000万高端会员，他们都是您潜在的合作伙伴。";
		} else {
	...
	private void share2QQ() {
		...
		if (isFromArchieve) {
			if (mProfile != null && mProfile.getUserInfo() != null) {
				...
				targetUrl = "http://r.renhe.cn/"+sharesid;;
			} else {
				Toast.makeText(this, "分享失败", Toast.LENGTH_SHORT).show();
				return;
			}
	...
	private void doShareToQQ(final Bundle params) {
		...
		final Activity activity = DialogActivity.this;
			...
			public void run() {
				...
				mQQShare.shareToQQ(activity, params, new IUiListener() {
					...
					public void onError(UiError e) {
						...
						Toast.makeText(DialogActivity.this, "分享失败", Toast.LENGTH_SHORT).show();
					}
