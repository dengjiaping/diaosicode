...
import com.itcalf.renhe.utils.HttpUtil;
   ...
   * @author wangning
   */
public class EditSummaryInfoProfessionTask  extends BaseAsyncTask<MessageBoardOperation> {
	private Context mContext;
	public EditSummaryInfoProfessionTask(Context mContext) {
		super(mContext);
	...
	protected MessageBoardOperation doInBackground(String... params) {
		...
		reqParams.put("professional", params[2]);// 
		try
		{
			MessageBoardOperation mb = (MessageBoardOperation) HttpUtil.doHttpRequest(
					Constants.Http.EDITPROFESSION, reqParams,
